...
class UserMeasures:
    ...
    def get_use_time(self):
        self.cur.execute("select email from FormalSurveyResults")
        emails = self.cur.fetchall()
        for email in emails:
            self.cur.execute(
                "select TIMESTAMPDIFF(DAY, CreationDate, '2018-04-15 00:00:00') from Users where Email = '%s'" % email[0])
            use_time = self.cur.fetchone()
            if use_time is None:
                print email[0]
            self.cur.execute(
                "insert into RespondentMeasures set email = '%s', Membership = '%s';" % (email[0], str(use_time[0])))
            self.conn.commit()

    def update_user_id(self):
        self.cur.execute("select rm.Email, u.Id from RespondentMeasures rm, Users u where rm.Email = u.Email;")
        emails = self.cur.fetchall()
        for email in emails:
            self.cur.execute("update RespondentMeasures set UserId = '%s' WHERE Email = '%s';" % (email[1], email[0]))
            self.conn.commit()
    ...
    def update_gender(self):
        with open("****") as seed:
            ...
            next(reader, None)
            for line in reader:
                self.cur.execute("update FormalSurveyResults set Gender = '%s' where Email = '%s'" % (line[3],line[1]))
                self.conn.commit()
    ...
    def count_individual_motivations(self):
        self.cur.execute("select Learning, HelpOthers, Enjoyment, PersonalObligation, IdentityConstruction, WorkSupport, GainReputation, GainRecognition, Reciprocity, VirtualRewards, SignalCapability, JobProspects,Email from FormalSurveyResults")
        motivations = self.cur.fetchall()
        for motivation in motivations:
            number = 0
            for i in range(0,12):
                if int(motivation[i]) >= 4:
                    number += 1
            self.cur.execute("update FormalSurveyResults set nMotivations = '%s' where Email = '%s';" % (str(number), motivation[12]))
            self.conn.commit()