...
class Users:
    ...
    def get_user_email(self):
        remote_conn = MySQLdb.connect(host='****', user='root', passwd='******', db='stackoverflow2012')
        remote_cur = remote_conn.cursor()
        self.cur.execute("select id from Users where reputation >= 1000 and id >= 5628")
        ids = self.cur.fetchall()
        i = 1
        for id in ids:
            print i
            remote_cur.execute(
                "select gh_u.login, gh_u.email from stackoverflow2012.users so_u, ghtorrent.users gh_u where so_u.id = '%s' and so_u.EmailHash = md5(gh_u.email) limit 1" %
                id[0])
            gh_account = remote_cur.fetchone()
            if gh_account is not None:
                self.cur.execute("update Users set GitHubAccount = '%s', Email = '%s' where Id = '%s'" % (
                    str(gh_account[0]), str(gh_account[1]), id[0]))
                self.conn.commit()
            i += 1

    def update_local_user(self):
        remote_conn = MySQLdb.connect(host='10.107.10.110', user='root', passwd='******', db='stackoverflow2012')
        remote_cur = remote_conn.cursor()
        remote_cur.execute("select id, GitHubAccount, Email from users where Email is not NULL;")
        emails = remote_cur.fetchall()
        i = 1
        for email in emails:
            print i
            self.cur.execute(
                "update Users set GitHubAccount = '%s', Email = '%s' where id = '%s'" % (email[1], email[2], email[0]))
            self.conn.commit()
            i += 1
    ...
    def obtain_suggested_edits(self):
